...
export default function EventStormingBoard({ sessionId }: { sessionId: string }) {
  ...
  const [linkPositions, setLinkPositions] = useState<Record<string, { x: number; y: number; left: number; right: number }>>({});
  ...
  useEffect(() => {
    const updatePositions = () => {
      ...
      const next: Record<string, { x: number; y: number; left: number; right: number }> = {};
      container.querySelectorAll<HTMLElement>("[data-storm-card]").forEach((element) => {
        const id = element.dataset.stormCard;
        if (!id) return;
        const rect = element.getBoundingClientRect();
        next[id] = { x: rect.left - bounds.left + rect.width / 2, y: rect.top - bounds.top + rect.height / 2, left: rect.left - bounds.left, right: rect.right - bounds.left };
      });
